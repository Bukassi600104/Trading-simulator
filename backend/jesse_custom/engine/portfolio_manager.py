...
- Sends margin call warnings
- Batches price updates for performance
"""
...
class PortfolioManager:
            ...
            for user_id, portfolio in self._portfolios.items():
                ...
                if position and position.is_open:
                    ...
                    if position.check_liquidation():
                        logger.warning(
                            f"Liquidation triggered for user "
                            f"{user_id} on {symbol}"
                        )
                        liquidated_symbols = portfolio.update_prices(
                            {symbol: price}
                        )
                        if liquidated_symbols:
                            liquidated_users.append(user_id)
                    elif position.margin_call_warning:
                        # Send margin call warning via WebSocket
                        await self._notify_margin_call(
                            user_id, symbol, position
                        )

                    # Notify subscribers of portfolio update
        ...
        async with self._lock:
            for user_id, portfolio in self._portfolios.items():
                ...
                if liquidated_symbols:
                    ...
                    logger.warning(
                        f"User {user_id} liquidated on: {liquidated_symbols}"
                    )

                # Check margin calls
                if portfolio.has_margin_call:
                    for symbol in SUPPORTED_SYMBOLS:
                        pos = portfolio.get_position(symbol)
                        if pos and pos.is_open and pos.margin_call_warning:
                            await self._notify_margin_call(
                                user_id, symbol, pos
                            )
    ...
    async def subscribe_to_updates(
        self, user_id: uuid.UUID, queue: asyncio.Queue
    ) -> None:
    ...
    async def _notify_portfolio_update(
        self, user_id: uuid.UUID, portfolio: UserPortfolio
    ) -> None:
    ...
    async def _notify_margin_call(
        self,
        user_id: uuid.UUID,
        symbol: str,
        position: UserPosition,
    ) -> None:
        """Send margin call warning to subscribed WebSocket clients."""
        if user_id in self._update_subscribers:
            try:
                await self._update_subscribers[user_id].put(
                    {
                ...
                logger.error(f"Failed to send margin call warning: {e}")
        ...
        try:
            async with async_session_maker() as session:
                ...
                if db_portfolio:
                    ...
                    db_portfolio.max_drawdown_watermark = (
                        portfolio.max_equity_watermark
                    )
                    db_portfolio.is_liquidated = portfolio.is_liquidated
                    db_portfolio.is_active = portfolio.is_active
                else:
                    logger.debug(
                        f"Portfolio for user {user_id} not in DB yet"
                    )

                await session.commit()
                logger.debug(
                    f"Synced portfolio for user {user_id} to database"
                )
        except Exception as e:
        ...
        try:
            async with async_session_maker() as session:
                ...
                if db_portfolio:
                    ...
                    self._portfolios[user_id] = portfolio
                    logger.info(
                        f"Loaded portfolio for user {user_id} from database"
                    )
                    return portfolio
    ...
    def get_stats(self) -> dict:
        ...
        active_count = sum(
            1 for p in self._portfolios.values() if p.is_active
        )
        liquidated_count = sum(
            1 for p in self._portfolios.values() if p.is_liquidated
        )
        ...
        return {
            ...
            "current_prices": {
                k: str(v) for k, v in self._current_prices.items()
            },
